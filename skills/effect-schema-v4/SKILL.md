...
Class constructors and `makeUnsafe` accept an optional second argument with `MakeOptions`:
...
class User extends Schema.Class<User>('User')({
	...
	role: Schema.String.pipe(
		Schema.withConstructorDefault(() => Option.some('viewer'))
	)
}) {}
...
// Also available on makeUnsafe and makeOption:
const user3 = User.makeUnsafe({ name: 'Alice' }, { disableChecks: true });
const user4 = User.makeOption({ name: 'Alice' }, { disableChecks: true });
```
